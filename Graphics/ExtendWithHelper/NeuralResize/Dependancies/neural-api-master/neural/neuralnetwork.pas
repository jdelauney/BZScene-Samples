...
begin
  ...
  if ((FStride = FPoolSize) and (FPadding = 0)) then
  begin
    Writeln('prev output size X :', pPrevLayer.Output.SizeX);
    Writeln('prev output size Y :', pPrevLayer.Output.SizeY);

    //if (pPrevLayer.Output.SizeX >= pPrevLayer.Output.SizeY) then
    //  MaxInputDivPool := pPrevLayer.Output.SizeX
    //else
    //  MaxInputDivPool := pPrevLayer.Output.SizeY;

    MaxInputDivPool := Max(pPrevLayer.Output.SizeX, pPrevLayer.Output.SizeY);// + 1;
    SetLength(FInputDivPool, MaxInputDivPool);
    Writeln('PoolSize :', FPoolSize);
    Writeln('MaxInputDivPool : ', MaxInputDivPool);
    for MaxInputDivPoolCnt := 0 to MaxInputDivPool-1 do
    begin
      Writeln('Count : ', MaxInputDivPoolCnt);

      FInputDivPool[MaxInputDivPoolCnt] := MaxInputDivPoolCnt div FPoolSize;
    end;
  end;
end;

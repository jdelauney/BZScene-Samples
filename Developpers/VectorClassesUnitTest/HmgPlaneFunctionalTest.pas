...
type
  ...
  THmgPlaneFunctionalTest = class(TVectorBaseTestCase)
    protected
      ...
      vt5 : TBZVector;
      ph1,ph2,ph3     : TBZHmgPlane; //TBZHmgPlaneHelper;
...
begin
  ...
  AssertTrue('TBZHmgPlane:Create3Vec:sub1 Point 1 does not lie on plane', IsEqual(fs1,0, 1e-5));
  fs1 := ph1.A * vt2.X + ph1.B * vt2.Y + ph1.C * vt2.Z + ph1.D;
  AssertTrue('TBZHmgPlane:Create3Vec:sub2 Point 2 does not lie on plane', IsEqual(fs1,0, 1e-5));
  fs1 := ph1.A * vt3.X + ph1.B * vt3.Y + ph1.C * vt3.Z + ph1.D;
  AssertTrue('TBZHmgPlane:Create3Vec:sub3 Point 3 does not lie on plane', IsEqual(fs1,0, 1e-5));
  fs1 := ph1.A * vt4.X + ph1.B * vt4.Y + ph1.C * vt4.Z + ph1.D;
  AssertFalse('TBZHmgPlane:Create3Vec:sub4 Point 4 does should not lie on plane', IsEqual(fs1,0, 1e-5));
end;
...
begin
  ...
  ph2.Create(vt3,norm);
  AssertTrue('TBZHmgPlane:CreatePointNorm:Sub1 planes do not match', compare(ph1,ph2, 1e-5));

  fs1 := ph2.A * vt1.X + ph2.B * vt1.Y + ph2.C * vt1.Z + ph2.D;
  AssertTrue('TBZHmgPlane:Create3Vec:sub2 Point 1 does not lie on plane', IsEqual(fs1,0, 1e-5));
  fs1 := ph2.A * vt2.X + ph2.B * vt2.Y + ph2.C * vt2.Z + ph2.D;
  AssertTrue('TBZHmgPlane:Create3Vec:sub3 Point 2 does not lie on plane', IsEqual(fs1,0, 1e-5));
  ...
  ph2.Create(vt3,norm);
  fs1 := ph2.A * vt1.X + ph2.B * vt1.Y + ph2.C * vt1.Z + ph2.D;
  AssertTrue('TBZHmgPlane:Create3Vec:sub4 Point 1 does not lie on plane', IsEqual(fs1,0, 1e-5));

end;

procedure THmgPlaneFunctionalTest.TestDefaultPlane;
begin
  AssertTrue('TBZHmgPlane:NormalizeSelf:Sub1 planes do not match', IsEqual(ph1.x,0));
  AssertTrue('TBZHmgPlane:NormalizeSelf:Sub1 planes do not match', IsEqual(ph1.y,0));
  AssertTrue('TBZHmgPlane:NormalizeSelf:Sub1 planes do not match', IsEqual(ph1.z,1));
  AssertTrue('TBZHmgPlane:NormalizeSelf:Sub1 planes do not match', IsEqual(ph1.W,-4.482));
end;
...
begin
  ...
  ph2.Create(vt4,norm);
  AssertTrue('TBZHmgPlane:NormalizeSelf:Sub1 planes do not match', compare(ph1,ph2));
  norm := norm * 3.56;
  ph2.Create(vt4,norm);
  AssertFalse('TBZHmgPlane:NormalizeSelf:Sub2 planes should not match', compare(ph1,ph2));
  ph2.Normalize;
  AssertTrue('TBZHmgPlane:NormalizeSelf:Sub3 planes do not match', compare(ph1,ph2, 1e-5));
end;
...
//  norm.Create(4,4,4);
  norm := norm * 3.56;
  ph2.Create(vt4,norm);
  AssertFalse('TBZHmgPlane:Normalize:Sub1 planes should not match', compare(ph1,ph2));
  ph3 := ph2.Normalized;
  AssertTrue('TBZHmgPlane:NormalizeSelf:Sub2 planes do not match', compare(ph1,ph3, 1e-5));
...
begin
   ...
   AssertTrue('TBZHmgPlane:DistanceToPoint:Sub1 Plane not suitable for test', (Abs(ph1.W) > 1));

   fs1 := ph1.Distance(vt5);
   AssertTrue('TBZHmgPlane:DistanceToPoint:Sub2 Lengths do not match', IsEqual(ph1.W, fs1, 1e-5));
   vt5 := ph1.AsVector * -ph1.W;
   vt5.W := 1;
   fs1 := ph1.Distance(vt5);
   AssertTrue('TBZHmgPlane:DistanceToPoint:Sub3 Lengths do not match', IsEqual(0, fs1, 1e-5));
   vt5 := ph1.AsVector * -ph1.W * 2;
   vt5.W := 1;
   fs1 := ph1.Distance(vt5);
   AssertTrue('TBZHmgPlane:DistanceToPoint:Sub4 Lengths do not match', IsEqual(-ph1.W, fs1, 1e-5));
end;
...
begin
  ...
  AssertTrue('TBZHmgPlane:DistanceToPoint:Sub2 Lengths do not match', IsEqual(Abs(ph1.W), fs1, 1e-5));
  vt5 := ph1.AsVector * -ph1.W;
  vt5.W := 1;
  fs1 := ph1.AbsDistance(vt5);
  AssertTrue('TBZHmgPlane:DistanceToPoint:Sub3 Lengths do not match', IsEqual(0, fs1, 1e-5));
  ...
  vt5.W := 1;
  fs1 := ph1.AbsDistance(vt5);
  AssertTrue('TBZHmgPlane:DistanceToPoint:Sub4 Lengths do not match', IsEqual(Abs(ph1.W), fs1, 1e-5));
end;
...
begin
   ...
   // is this plane any use to this test.
   AssertTrue('TBZHmgPlane:DistanceToSphere:Sub1 Plane not suitable for test', (Abs(ph1.W) > 1));

   AssertTrue('TBZHmgPlane:DistanceToSphere:Sub2 Lengths do not match', IsEqual(ph1.W + 2, fs1, 1e-5));
   vt5 := ph1.AsVector * -ph1.W;
   vt5.W := 1;
   fs1 := ph1.Distance(vt5, 6);
   AssertTrue('TBZHmgPlane:DistanceToSphere:Sub3 Lengths do not match', IsEqual(0, fs1, 1e-5));
   vt5 := ph1.AsVector * -ph1.W * 2;
   vt5.W := 1;
   fs1 := ph1.Distance(vt5,2);
   AssertTrue('TBZHmgPlane:DistanceToSphere:Sub4 Lengths do not match', IsEqual(-ph1.W - 2, fs1, 1e-5));
end;
...
begin
   ...
   // is this plane any use to this test.
   AssertTrue('TBZHmgPlane:IsInHalfSpace:Sub1 Plane not suitable for test', (Abs(ph1.W) > 1));

   if ph1.W > 0 then // origin should be in half space
     AssertTrue('TBZHmgPlane:InHalfSpace:Sub2 half space failed', nb)
   else
     AssertFalse('TBZHmgPlane:InHalfSpace:Sub2 half space failed', nb);

   vt5 := ph1.AsVector * (abs(ph1.W) * 2);
   vt5.W := 1;
   nb := ph1.IsInHalfSpace(vt5);
   if ph1.W > 0 then // origin should be in half space
     AssertFalse('TBZHmgPlane:InHalfSpace:Sub3 half space failed', nb)
   else
     AssertTrue('TBZHmgPlane:InHalfSpace:Sub3 half space failed', nb);
end;
...
var
  sp: TBZBoundingSphere;
  ct: TBZSpaceContains;
begin

  vt5.Create(0,0,0,1);
  // is this plane any use to this test.
  AssertTrue('TBZHmgPlane:ContainsBSphere:Sub1 Plane not suitable for test', (Abs(ph1.W) > 2));
  // sphere should not be touching plane
  sp.Create(vt5,1);
  ct := ph1.Contains(sp);
  AssertTrue('TBZHmgPlane:ContainsBSphere:Sub2 Plane should not contain BSphere', (ScNoOverlap = ct));
  // grow sphere so it does touch the plane
  sp.Radius:= abs(ph1.W * 1.1);
  ct := ph1.Contains(sp);
  AssertTrue('TBZHmgPlane:ContainsBSphere:Sub3 Plane should partially contain BSphere', (ScContainsPartially = ct));

  // is vt5 instance still center instance
  // shrink sphere
  sp.Radius := 1;
  vt5 := ph1.AsVector * abs(ph1.W);  // set vt5 to sit on plane
  AssertFalse('TBZHmgPlane:ContainsBSphere:Sub4 Points are the same instance something changed', Compare(vt5,sp.Center));

  // small sphere center now sits on plane
  sp.Center := vt5;
  ct := ph1.Contains(sp);
  AssertTrue('TBZHmgPlane:ContainsBSphere:Sub5 Plane should contain BSphere', (ScContainsPartially = ct));

  vt5 := ph1.AsVector * (abs(ph1.W) * 2);  // set sphere to sit in other side of plane
  vt5.W := 1;
  sp.Center := vt5;
  ct := ph1.Contains(sp);
  AssertTrue('TBZHmgPlane:ContainsBSphere:Sub6 Plane should fully contain BSphere', (ScContainsFully = ct));
